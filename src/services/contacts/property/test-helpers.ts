...
export const PROPERTY_ID = "prop_123456789";

export function propertyFixture(overrides: Record<string, unknown> = {}) {
	return {
...
export function propertyListItemFixture(
	overrides: Record<string, unknown> = {},
) {
	const { object: _object, event: _event, ...rest } = propertyFixture(overrides);
	return rest;
}
...
export function listResponseFixture(overrides: Record<string, unknown> = {}) {
	return {
		object: "contact_property" as const,
		properties: [propertyListItemFixture()],
		total: 1,
		page: 1,
		limit: 10,
		event: "evt_list",
